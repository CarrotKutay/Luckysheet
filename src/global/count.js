...
export function countfunc() {
    ...
    if (isFinite(max) || isFinite(min)) {
        ret += "<span>"+locale_formula.sum+":" + update("#,##0.00", sum) + "</span>";
        ret += "<span>"+locale_formula.average+":" + update("#,##0.00", Math.round(sum / count * 10000) / 10000) + "</span>";
    }

    if (isFinite(max)) {
        ret += "<span>"+locale_formula.max+":" + update("#,##0.00", max) + "</span>";
    }

    if (isFinite(min)) {
        ret += "<span>"+locale_formula.min+":" + update("#,##0.00", min) + "</span>";
    }
